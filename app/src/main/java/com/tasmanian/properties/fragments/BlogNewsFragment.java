...
import com.tasmanian.properties.adapter.BlogAdapter;
...
public class BlogNewsFragment extends ParentFragment implements IItemHandler {
    ...
    public BlogAdapter adapter = null;
    ...
    private void getProertyData(int requestId, int currentNo) {
        ...
        try {
            if (adapter == null) {
                adapter = new BlogAdapter(propertUpdates, "");
                mRecyclerView.setAdapter(adapter);
            }

            String link = propertUpdates.getPropertyValue("blogapi");
            HTTPPostTask task = new HTTPPostTask(getActivity(), this);
            task.disableProgress();
            task.userRequest("", 1, link, "", 1);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void onFragmentChildClick(View view) {
        ...
        propertUpdates.bloggerData(item);
    ...
    public String getFragmentName() {
        return "Bloger";// getString(R.string.protections);
    }
        ...
        switch (requestType) {
            case 1:
                ...
                if (results != null) {
                    ...
                    Vector<Item> items = (Vector<Item>) item.get("latestnews_details");
