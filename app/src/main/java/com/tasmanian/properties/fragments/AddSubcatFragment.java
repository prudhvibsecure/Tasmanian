...
public class AddSubcatFragment extends ParentFragment implements IItemHandler {
    ...
    private SubcatSecAdapter adapter = null;
    ...
    public void getProertyData(int requestId, int currentNo) {

        getView().findViewById(R.id.catgr_txt).setVisibility(View.GONE);

        try {
            if (adapter == null) {
                adapter = new SubcatSecAdapter(propertUpdates, "");
                mRecyclerView.setAdapter(adapter);
            }

            String link = propertUpdates.getPropertyValue("sub_categories");
            JSONObject object = new JSONObject();
            object.put("slug", item.getAttribute("slug"));
    ...
    public void onFragmentChildClick(View view) {
        ...
        if (item.getAttribute("innercategory").equalsIgnoreCase("Yes")) {

            propertUpdates.showAddBusinessSubcatList(item);
        } else {
